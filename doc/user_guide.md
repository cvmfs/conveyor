...
A Conveyor system is made up of multiple components, implemented as subcommands of the `conveyor` executable:

1. **The client tools**

     The user can submit jobs to a Conveyor system using the `conveyor submit`
     command. This can be done from a machine that does not have the CernVM-FS
     server tools installed, such a CI builder node.
...
1. **The job server**
     ...
     It receives job submissions and queries from clients, and maintains a database with the status of all processed jobs.
...
Unlike the RPM package, the tarball has no installation scripts for configuration and systemd service files.
...
RabbitMQ is needed by all the Conveyor components.
...
An alternative configuration file can be specified with the `--config` parameter.
...
* `log_timestamps` - (bool) Include timestamps in logging output
...
* `name` - (string) A name to identify the worker. It defaults to the hostname and there is no check for uniqueness among multiple workers connected to the same server
...
An UUID is assigned to a job when it is submitted, and can later be used to query the status of the job with the `conveyor check` command, or to list the job as a dependency of another job.
...
Currently, only the `script` payload type has been implemented, but multiple types of payload will be added in the future.
